...
from model.DGFont.blocks import LinearBlock, Conv2dBlock, ResBlocks

import sys
sys.path.append('../..')

# from mmcv.ops import DeformConv2dPack as DCN
from model.DGFont.dcn import DCN
...
class Generator(nn.Module):
    def __init__(self, img_size=80, sty_dim=64, n_res=2, use_sn=False, mute=False, baseline_idx=2):
        ...
        self.decoder = Decoder(nf_dec, sty_dim, n_downs, n_res, self.decoder_norm, self.decoder_norm, 'relu', 'reflect',
                               use_sn=use_sn, baseline_idx=baseline_idx)
...
class Decoder(nn.Module):
    def __init__(self, nf_dec, sty_dim, n_downs, n_res, res_norm, dec_norm, act, pad, use_sn=False, mute=False,
                 baseline_idx=2):
        ...
        if baseline_idx == 0:  # old
            ...
            self.model.append(Conv2dBlock(nf, nf // 2, 5, 1, 2, norm=dec_norm, act=act, pad_type=pad,
                                          use_sn=use_sn))  # 2 scale x2 channel x 1/2 [128]
            # dcn_2: 2.1 scale x2 channel x1 [256]
            nf //= 2  # 128

            self.model.append(nn.Upsample(scale_factor=2))  # 3 scale x4 channel x1 [256]
            self.model.append(Conv2dBlock(2 * nf, nf // 2, 5, 1, 2, norm=dec_norm, act=act, pad_type=pad,
                                          use_sn=use_sn))  # 4 scale x4 channel x 1/4 [64]
            ...
            for _ in range(n_downs - 2):
                ...
                self.model.append(
                    Conv2dBlock(nf, nf // 2, 5, 1, 2, norm=dec_norm, act=act, pad_type=pad, use_sn=use_sn))
                nf //= 2

            self.model.append(nn.Upsample(scale_factor=2))
            self.model.append(
                Conv2dBlock(2 * nf, nf // 2, 5, 1, 2, norm=dec_norm, act=act, pad_type=pad, use_sn=use_sn))
            nf //= 2
            ...
            self.model.append(ResBlocks(n_res, nf, res_norm, act, pad, use_sn=use_sn))  # 0 scale x1 channel x1 [256]
            if baseline_idx == 2:
                self.model.append(Conv2dBlock(nf, nf, 5, 1, 2, norm=dec_norm, act=act, pad_type=pad,
                                              use_sn=use_sn))  # 2 scale x2 channel x 1/2 [256] # <--- baseline 2
            ...
            self.model.append(nn.Upsample(scale_factor=2))  # 3 scale x4 channel x1 [256]
            self.model.append(
                Conv2dBlock(2 * nf, nf // 2, 5, 1, 2, norm=dec_norm if self.baseline_idx != 3 else 'in', act=act,
                            pad_type=pad, use_sn=use_sn))  # 4 scale x4 channel x 1/4 [64]
            nf //= 2
